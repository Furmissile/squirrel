...
struct discord_component* build_button_container(struct sd_invite_info *invite_info)
{
  ...
  buttons->array[0] = (struct discord_component)
  {
    ...
    .custom_id = format_str(SIZEOF_CUSTOM_ID,
        "%c0_%ld", TYPE_INVITE, invite_info->owner_id),
  };
  buttons->array[1] = (struct discord_component)
  {
    ...
    .label = "Decline",
    .custom_id = format_str(SIZEOF_CUSTOM_ID,
        "%c1_%ld", TYPE_INVITE, invite_info->owner_id),
  };
        ...
        {
          {
            ...
            // .author = sd_msg_embed_author(username, avatar),
            .author = sd_msg_embed_author(
                event->user->username, 
...
{
  struct sd_invite_info *invite_info = resp->data;
  struct sd_message *discord_msg = invite_info->discord_msg;
...
void send_invite_dm(struct discord *client, struct discord_response *resp, const struct discord_channel *channel)
{
  ...
  discord_create_message(client, channel->id,
    &(struct discord_create_message)
    {
      ...
      {
        .array = build_button_container(invite_info),
        .size = 1
      }
    }, 
    &ret_msg);
  ...
  struct sd_message *discord_msg) 
{
  ...
  struct sd_invite_info *invite_info = calloc(1, sizeof(struct sd_invite_info));
  invite_info->discord_msg = discord_msg;
